...
import CandleStrikeChartPanel from './CandleStrikeChartPanel';
import { Activity } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const CandleStrikeBotChartGrid = ({ 
    symbols, 
    selectedBot, 
    botSettings, 
    previewMode, 
  ...
  // Single Chart Mode (Selection or Preview)
  if (activeSymbol) {
      return (
        <div className="h-full flex flex-col bg-[#1a1a1a] rounded-2xl border border-[#2a2a2a] p-4 overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 px-2 gap-4 sm:gap-0">
                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider">
                    {previewMode && !selectedBot ? 'Candle Strike Preview' : `Monitoring Bot: ${activeSymbol}`}
                </h3>
                <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto">
                    <TimeframeSelector />
                    <div className="flex items-center gap-2 shrink-0">
                        <span className="w-2 h-2 rounded-full bg-pink-500 animate-pulse"></span>
                        <span className="text-xs text-pink-500 font-mono">Connected</span>
                    </div>
                </div>
            </div>
            <div className="h-full min-h-[400px] relative">
                 <CandleStrikeChartPanel 
                    key={activeSymbol}
                    symbol={activeSymbol} 
                    timeframe={timeframe} 
                    botSettings={botSettings}
                    onCandleCountUpdate={onCandleCountUpdate}
                />
            </div>
        </div>
      );
      ...
      <div className="bg-[#1a1a1a] rounded-2xl p-8 border border-[#2a2a2a] h-full min-h-[500px] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400">
            No active Candle Strike bots. <br/>Configure a bot in the panel to see a preview.
          </p>
        </div>
      </div>
    );
...
export default CandleStrikeBotChartGrid;